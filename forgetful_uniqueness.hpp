...
// if a new element is inserted into a uniqueness set, it should also be
// inserted here. upon insertion here, the least recently used element will be
// removed from the uniqueness set if capacity is reached.
...
struct ForgetfulManager {
  ...
  // least recently used queue. back is next to be removed. front is most
  // recently used
  ...
  ForgetfulManager(size_t n,  //
    ...
    // it is required since n==0 means upon insertion of an element into the
    // uniqueness set, the element is immediately removed by this instance (in
    // ForgetfulManager::insert). meaning the returned iterator (by
    // ForgetfulSet::insert) would be invalid
  }

  // must be called before using and after all copies or moves of this instance
  void setup(T* obj) { this->obj = obj; }

  // remove expired elements. must be called before attempting insertion in
  // uniqueness set
  void remove_old() {
...
struct ForgetfulSet {
 ...
 public:
  ForgetfulSet(const Compare& comp,  //
               size_t n,
               typename decltype(lru)::duration_t expiry =
                   decltype(lru)::duration_t::zero())
      : s(comp),  //
        lru(n, expiry) {}
  ...
  auto insert(Key k) {
    ...
    static_assert(sizeof(typename decltype(lru)::refresh_handle) ==
                  sizeof(void*));
...
struct ForgetfulUnorderedSet {
 ...
 public:
  ForgetfulUnorderedSet(const Hash& hash,  //
    ...
    // prevent iters invalidation. +1 since element is inserted before one is
    // erased to maintain cap
